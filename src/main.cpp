...
void decrypt_image(const char* input_image, int message_length) {
 ...
 int main(int argc, char** argv) {
    ...
    try {
        Crypto crypto;
        std::string secretMessage = "TA Selma's assignment is a nightmare.";

        // Orijinal görüntüyü yükle
        GrayscaleImage original_image("puppy.png");
        int width = original_image.get_width();
        int height = original_image.get_height();
        int dimension = width * height;
        std::cout << "Original image dimensions: " << width << "x" << height << " = " << dimension << " pixels" << std::endl;

        // Mesajı şifrele (LSB array'e dönüştür)
        std::vector<int> LSB_array = crypto.encrypt_message(secretMessage);
        std::cout << "Total bits to embed: " << LSB_array.size() << std::endl;

        // Görüntünün yeterli olup olmadığını kontrol et
        if (LSB_array.size() > dimension) {
            throw std::runtime_error("Message is too large for the image.");
        }

        // Şifreli görüntüyü oluştur
        SecretImage secimg = crypto.embed_LSBits(original_image, LSB_array);
        secimg.save_to_file("si.png");
        std::cout << "Secret message embedded and saved to si.png" << std::endl;

        // Şifreli görüntüyü tekrar yükle
        SecretImage loaded_secimg("si.png");
        GrayscaleImage loaded_image = loaded_secimg.reconstruct();
        std::cout << "Loaded si.png for extraction." << std::endl;

        // Mesaj uzunluğunu belirle
        int message_length = secretMessage.size();

        // LSB'leri çıkar
        std::vector<int> extracted_LSBs = crypto.extract_LSBits(loaded_secimg, message_length);
        std::cout << "Extracted LSBs: ";
        for(auto bit : extracted_LSBs) {
            std::cout << bit;
        }
        std::cout << std::endl;

        // Mesajı deşifre et
        std::string decryptedMessage = crypto.decrypt_message(extracted_LSBs);
    ...
    // SecretImage nesnesini img1'den oluşturalım
    SecretImage secret_img(img1);

    // SecretImage'i bir dosyaya kaydedelim
    std::string filename = "flowers.dat";
    secret_img.save_to_file(filename);
    std::cout << "Secret image saved to file: " << filename << std::endl;

    // SecretImage'i dosyadan yükleyelim
    SecretImage loadedSecretImage = SecretImage::load_from_file(filename);
    std::cout << "Secret image loaded from file: " << filename << std::endl;

    // Yüklenen SecretImage'den GrayscaleImage'i yeniden oluşturalım
    GrayscaleImage reconstructed_img = loadedSecretImage.reconstruct();

    // Yeniden oluşturulan resmi bir dosyaya kaydedelim
    reconstructed_img.save_to_file("reconstructed_flowers.png");
    std::cout << "Reconstructed image saved to file: reconstructed_flowers.png" << std::endl;
  Filter filter;
    // Görüntü filtreleme işlemi uygulayalım (örneğin, negatif filtre)
    filter.apply_mean_filter(reconstructed_img);

    // Filtrelenmiş resmi kaydedelim
    reconstructed_img.save_to_file("filtered_flowers.png");
    std::cout << "Filtered image saved to file: filtered_flowers.png" << std::endl;

    // Filtrelenmiş resmi SecretImage formatına geri kaydedelim
    loadedSecretImage.save_back(reconstructed_img);

    // Güncellenmiş SecretImage'i yeni bir dosyaya kaydedelim
    loadedSecretImage.save_to_file("updated_flowers.txt");
    std::cout << "Updated secret image saved to file: updated_flowers.txt" << std::endl;
    GrayscaleImage img2("reconstructed_flowers.png");
    if(img1==img2) {
        std::cout << "Images are equal." << std::endl;
    }
    */

    // Yüklenen SecretImage ile görüntüyü yeniden oluştur
    /*GrayscaleImage img("puppy.png");
    Filter filter;
     filter.apply_gaussian_smoothing(img,21,4);
    img.save_to_file("puppy2.png");
    GrayscaleImage  img1("puppy2.png");

    GrayscaleImage img2("gaussian_filtered_puppy_21x21_4.png");
    if (img1==img2) {
        std::cout << "Images are equal." << std::endl;
    }else {
     ...
     GrayscaleImage img("creep.png");
    Filter filter;
    filter.apply_mean_filter(img,11);
    img.save_to_file("creep11.png");
    GrayscaleImage  img1("creep11.png");

    GrayscaleImage img2("mean_filtered_creep_11x11.png");
    if (img1==img2) {
        std::cout << "Images are equal." << std::endl;
    }else {
        std::cout << "Images are not equal." << std::endl;
...
/*
    GrayscaleImage img("flowers.png");
    Filter filter;
    filter.apply_unsharp_mask(img,9,1);
    img.save_to_file("flowers1.png");
    GrayscaleImage  img1("flowers1.png");

    GrayscaleImage img2("unsharp_filtered_flowers_9x9_1.png");
    if (img1==img2) {
        std::cout << "Images are equal." << std::endl;
    }else {
        std::cout << "Images are not equal." << std::endl;
